...
graph TD
  ...
  B1 -- No --> C;
  B2 --> C["Update .ai/TASKS.md with ALL Planned Tasks"];
  ...
  S{User asks to archive?} --> T["Find completed/failed tasks in .ai/tasks/"];
  T --> U["Move task files to .ai/memory/tasks/"];
  U --> U_HOOK["Check & Run 'task_archival' Hooks"];
  U_HOOK --> V["Append to .ai/memory/TASKS_LOG.md"];
  V --> W["Remove entries from .ai/TASKS.md"];
```
